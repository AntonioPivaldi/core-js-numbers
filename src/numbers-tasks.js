...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos(
    scalarComposition / (firstVectorLength * secondVectorLength)
  );
}
...
function getLastDigit(n) {
  // I'm 100% sure this would work for any needs. Matematik...
  return `${n}`.slice(-1) * 1;
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 0; i < index; i += 1) {
    fiboRow.push(fiboRow[i] + fiboRow[i + 1]);
  }
  return fiboRow[index];
}
...
function getSumOfDigits(n) {
  return `${n}`.split('').reduce((el, acc) => acc * 1 + el * 1, 0);
}
